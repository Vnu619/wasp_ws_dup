...
portMotorBL = '/dev/ttyUSB1'
portMotorBR = '/dev/ttyUSB3'
...
orienbusFL = orienbus.OrienBus(portMotorFL)
orienbusFR = orienbus.OrienBus(portMotorFR)
orienbusBL = orienbus.OrienBus(portMotorBL)
orienbusBR = orienbus.OrienBus(portMotorBR)
...
class WaspController(object):
	...
	self.BR_motor_id  = 3
        self.BL_motor_id = 4
	self.FL = orienbusFL.initialize(self.FL_motor_id)
	self.FR = orienbusFR.initialize(self.FR_motor_id)
	#self.BR = orienbusBR.initialize(self.BR_motor_id)
	#self.BL = orienbusBL.initialize(self.BL_motor_id)
	self.ydot=0
	self.xdot=0
	self.thetadot=0  
	self.roll = 0
	...
	#v_fl_real = v_fl_real/60*R*3.14/50
        ...
        v_br_real = 0 #self.BR.readSpeed()/60*R*(3.14/50)
        v_bl_real = 0 #self.BL.readSpeed()/60*R*(3.14/50)
