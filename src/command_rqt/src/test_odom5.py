...
class WaspController(object):
	...
	self.BR_motor_id  = 3
        self.BL_motor_id = 4
	self.FL = orienbus_new.orienMotor(portMotor,self.FL_motor_id)
	self.FR = orienbus_new.orienMotor(portMotor,self.FR_motor_id)
	self.BR = orienbus_new.orienMotor(portMotor,self.BR_motor_id)
	self.BL = orienbus_new.orienMotor(portMotor,self.BL_motor_id)
	self.ydot=0
	self.xdot=0
	self.thetadot=0  
