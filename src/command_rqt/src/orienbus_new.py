...
class orienMotor:
	...
	def writeSpeed(self, speed):
		...
		if (speed >= _MIN_RPM and speed <= _MAX_RPM):
			...
			request_2 = bytes(address+'\x10\x00\x7d\x00\x01\x02\x00\x0a')+_calculate_crc_string(bytes(address+'\x10\x00\x7d\x00\x01\x02\x00\x0a'))
			self._communicate(request_2)
 		elif speed <= -_MIN_RPM and speed >= -_MAX_RPM:			
			value = bytearray.fromhex('{:04x}'.format(-speed))
			request_1 = bytes(address+'\x10\x04\x85\x00\x01\x02'+value)+_calculate_crc_string(bytes(address+'\x10\x04\x85\x00\x01\x02'+value))
			self._communicate(request_1)
			request_2 = bytes(address+'\x10\x00\x7d\x00\x01\x02\x00\x12')+_calculate_crc_string(bytes(address+'\x10\x00\x7d\x00\x01\x02\x00\x12'))
			self._communicate(request_2)
		else:
			
			self._communicate(bytes(address+'\x10\x00\x7d\x00\x01\x02\x00\x22')+_calculate_crc_string(bytes(address+'\x10\x00\x7d\x00\x01\x02\x00\x22')))

	def readSpeed(self):
		self._print_debug(str(self.serial.isOpen()))
		address = chr(self._slave_address)
		speed = struct.unpack('>H',self._communicate(bytes(address+'\x03\x00\xcf\x00\x01')+_calculate_crc_string(bytes(address+'\x03\x00\xcf\x00\x01')))[3:5])[0]
		if self._debug:
			speed2 = struct.unpack('>H',self._communicate(bytes(address+'\x03\x00\xce\x00\x01')+_calculate_crc_string(bytes(address+'\x03\x00\xce\x00\x01')))[3:5])[0]
			self._print_debug("raw speed reading1 is:{}".format(speed))
			self._print_debug("raw speed reading2 is:{}".format(speed2))
		if speed >10000:
			speed = speed - 65535
		return speed
