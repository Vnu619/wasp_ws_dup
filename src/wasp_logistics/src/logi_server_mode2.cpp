...
float dist=0,right_min=0,left_min=0,x_r,x_l,y_r,y_l, angle=0.0;
float pi =3.14;
...
std::string footprint_string = "[[-0.36, -0.65], [-0.36, 0.65], [0.36, 0.65], [0.36, -0.65]]";
std::string footprint_normal = "[[-0.32, -0.28], [-0.32, 0.28], [0.32, 0.28], [0.32, -0.28]]";
std::string footprint_laser = "[[-0.45, -0.75], [-0.45, 0.75], [0.45, 0.75], [0.45, -0.75]]";
		...
		if(!lg.isActive() || lg.isPreemptRequested())
		{
     	...
     	if (goal->liftup==1 && goal-> liftdown == 0)
     	{
			...
			if (linear_dist && (abs(this->odom_orientationz- odom_start_orientation) > angle) )
			{
				...
				if(abs(ros::Time::now().toSec()-last_time.toSec())<60)
				{
					logistics_command.liftup = 1.0;
					std::cout<<"\ntimerstarted for 80 seconds";
					success =false;
				}
				else if(abs(ros::Time::now().toSec()-last_time.toSec())>=60)
				{
					logistics_command.liftup = 0.0;
