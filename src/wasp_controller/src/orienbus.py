...
_R0_STATUS_REGISTER = 372
R0status = 0

_EXTERRORSTATUS_REGISTER = 368
ExtErrorStatus = 0

_ALMRSTSTATUS_REGISTER = 369
AlmRstStatus = 0

_ROTATIONMETER_REGISTER = 315
rotationmeter = 0

_RSTROTATIONMETER_REGISTER = 431
...
class ModBus(object):
	...
	def readR0status(self):

		global R0status
		R0status = self.instrument.read_register(_R0_STATUS_REGISTER)

		return R0status

	def readExtErrorStatus(self):

		global ExtErrorStatus
		ExtErrorStatus = self.instrument.read_register(_EXTERRORSTATUS_REGISTER)

		return ExtErrorStatus

	def readAlmRstStatus(self):

		global AlmRstStatus
		AlmRstStatus = self.instrument.read_register(_ALMRSTSTATUS_REGISTER)

		return AlmRstStatus

	def readRotationmeter(self):

		global rotationmeter
		rotationmeter = self.instrument.read_register(_ROTATIONMETER_REGISTER)

		return rotationmeter

	def resetRotationmeter(self):
		if self.instrument.read_register(_RSTROTATIONMETER_REGISTER) == 0:
			self.instrument.write_register(_RSTROTATIONMETER_REGISTER, 1)
		elif self.instrument.read_register(_RSTROTATIONMETER_REGISTER) == 1:
			self.instrument.write_register(_RSTROTATIONMETER_REGISTER, 0)
			self.instrument.write_register(_RSTROTATIONMETER_REGISTER, 1)
