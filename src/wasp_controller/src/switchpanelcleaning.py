...
class testing(object):
    ...
    def __init__(self):
        ...
        self.dist_near = 0.480
        self.dist_switch = 0.380
        self.dist_away = 0.520
        # self.state = 0
        self.flag = 0
        self.vx = 0
        self.vy = 0
...
#                        cmd_vel.angular.z = self.w
        ...
        if self.state == 3:
            ...
            print('cycle: ', self.cycle)
            if self.d_dist <= self.dist_switch:
                cmd_vel = Twist()
                self.vy = 0
                cmd_vel.linear.y = self.vy
                self.wheel_speed_pub.publish(cmd_vel)
                self.state = 3.1
            elif self.d_dist > self.dist_switch:
                # print(self.d_dist)
                self.vy = 0.05
                cmd_vel = Twist()
                cmd_vel.linear.y = self.vy
